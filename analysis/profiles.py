...
# Back half of race starts at window N_WINDOWS // 2 (hour 12).
# Profiles are labelled by slope of pace curve over this back half.
_BACK_HALF_START = N_WINDOWS // 2

# n_init=20: more restarts than the sklearn default (10) for stability on this
# small dataset (~145 training samples per LOYO fold).
_KMEANS_N_INIT = 20

# Fallback pace used when a window has no valid pace data (5 min/lap = slow walk).
_FALLBACK_PACE_SEC = 300.0
...
class PaceProfiler:
    ...
    def __init__(self) -> None:
        ...
        self.profile_curves_: np.ndarray | None = None  # shape (n_clusters, N_WINDOWS)
        self.profile_labels_: list[str] = []
        self.n_clusters_: int = 0
        self.n_components_: int = 0
        # Confidence bounds from LOYO backtest: shape (N_WINDOWS, 2) → (p10, p90) error km
    ...
    def fit(
        ...
        X = X[valid]
        _impute_nan_cols(X, np.nanmean(X, axis=0))
        ...
        self.kmeans_ = KMeans(n_clusters=n_clusters, random_state=42, n_init=_KMEANS_N_INIT)
    ...
    def _assign_labels(self) -> None:
        """
        Label profiles by the slope of their back half (windows _BACK_HALF_START–47).
        Steeper positive slope (more fade) → "Aggressive". Flattest → "Steady".
        """
        curves = self.profile_curves_
        back_slopes = []
        for i in range(self.n_clusters_):
            back = curves[i, _BACK_HALF_START:]
    ...
    def predict_distance(
        ...
        partial_vector: shape (N_WINDOWS,), NaN for unobserved windows.
        ...
        return self._count_laps(full_abs)
    ...
    def _fit_best_profile(
        ...
        scale = (obs_abs · curve_obs) / (curve_obs · curve_obs)  — OLS no intercept.
        """
        obs_idx = np.where(~np.isnan(partial_vector))[0]
        obs_abs = partial_vector[obs_idx] * mean_pace

        best_idx, best_scale, best_residual = 0, mean_pace, np.inf
        for k in range(self.n_clusters_):
            curve_obs = self.profile_curves_[k][obs_idx]
    ...
    def _blend_trajectory(
        ...
        """
        Build full N_WINDOWS absolute pace array:
    ...
    def predict_trajectory(
        ...
        """
        Return (observed_abs, full_abs) — absolute sec/lap for all N_WINDOWS windows.
    ...
    def _count_laps(pace_by_window: np.ndarray) -> float:
        """Simulate laps through 24h given per-window average pace (sec/lap). Returns total km."""
        ...
        for w in range(N_WINDOWS):
            ...
            if np.isnan(pace) or pace <= 0:
                pace = _FALLBACK_PACE_SEC
            window_budget = min(remaining, float(WINDOW_SEC))
            total_laps += window_budget / pace
            remaining -= window_budget
        from analysis.features import LAP_KM
        return total_laps * LAP_KM

    def _prepare_X(self, feature_df: pd.DataFrame) -> np.ndarray:
        X = feature_df[WINDOW_COLS].to_numpy(dtype=float)
        return _impute_nan_cols(X, self.scaler_.mean_)
...
def select_n_clusters(
    ...
    for k in range(2, max_k + 1):
        km = KMeans(n_clusters=k, random_state=42, n_init=_KMEANS_N_INIT)
